  ...
  useEffect(() => {
    if (visible) {
      ...
      uncompletedTasks.forEach(t => {
        const rec = recommendations.find(r => r.title === t.title);
        ...
        {uncompletedTasks.length === 0 ? (
          <View className="py-12 items-center justify-center gap-4">
            ...
            <Text className="text-sm text-on-surface-variant text-center font-medium leading-relaxed px-6">
              You checked off all daily board goals today. No overflow restructuring is necessary. Outstanding work!
            </Text>
            <Pressable 
              onPress={onClose}
          ...
          <View className="gap-4 flex-1">
            <Text className="text-on-surface-variant font-medium text-xs leading-relaxed">
              You have **{uncompletedTasks.length} uncompleted tasks** remaining. To stay within tomorrow's strict **8-point capacity limit**, the AI recommends moving non-critical items back to the Backlog.
            </Text>
