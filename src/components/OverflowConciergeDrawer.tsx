...
export function OverflowConciergeDrawer({ visible, onClose }: OverflowConciergeDrawerProps) {
  ...
  const boardTasks = tasks.filter(t => t.status === 'today' && !t.completed);
  ...
  const handleConfirm = () => {
    ...
    Object.entries(actions).forEach(([id, action]) => {
      if (action === 'backlog') {
        ...
        updateTask(id, { status: 'today' }); // Keep on Board
      }
    });
      ...
      >
        ...
        <View className="flex-row justify-between items-center mb-4">
          <View className="flex-row items-center gap-2">
            ...
            <View>
              ...
              <Text className="text-xs font-black uppercase tracking-widest text-primary">Rollover Assistant</Text>
            </View>
          </View>
          <Pressable 
            onPress={onClose}
        ...
        {boardTasks.length === 0 ? (
          <View className="py-12 items-center justify-center gap-4">
            ...
            <Text className="text-sm text-slate-500 dark:text-slate-400 text-center font-medium leading-relaxed px-6">
          ...
          <View className="gap-4 flex-1">
            <Text className="text-slate-500 dark:text-slate-400 font-medium text-xs leading-relaxed">
              You have {boardTasks.length} uncompleted tasks remaining. To stay within your daily capacity limit, your AI assistant recommends moving non-critical items back to the Backlog.
            </Text>
                    ...
                    >
                      <View className="flex-row justify-between items-start">
                        <View className="flex-1 pr-2">
                          ...
                          <Text className="text-xs font-black uppercase tracking-wider text-slate-650 dark:text-slate-300">
                        ...
                        >
                          {action === 'keep' ? (
                            <>
                              ...
                              <Text className="text-xs font-black uppercase tracking-wider text-secondary">Keep Tomorrow</Text>
                            </>
                          ) : (
                            <>
                              <Inbox size={12} color="#73777f" />
                              <Text className="text-xs font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">Send to Backlog</Text>
                            </>
                          )}
                        </Pressable>
                      </View>
                      ...
                      <View className="bg-surface-container-high/40 p-3 rounded-2xl border border-outline-variant/10">
                        <Text className="text-xs text-slate-500 dark:text-slate-400 leading-normal font-medium">
                          {isMust 
